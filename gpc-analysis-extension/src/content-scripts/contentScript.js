...
const fpjsDetect = `
  try {
    const needles = ["fpjscdn.net/v3/", "fpjscdn.net/v4/", "fpjscdn.net/v5/"];
    let fpjsDetected = false;
    let detectedUrls = [];
    
    try {
      for (const resources of performance.getEntriesByType("resource")) {
        for (let i = 0; i < needles.length; i++) {
          if(String(resources.name).includes(needles[i])) {
            fpjsDetected = true;
            fpjsUrls.push(resources.name);
          }
        }
      }
    } catch (e) {}
      ...
      try {
        const po = new PerformanceObserver(list => {
          for (const entry of list.getEntries()) {
            for (let i = 0; i < needles.length; i++) {
              if(String(entry.name).includes(needles[i])) {
                fpjsDetected = true;
                fpjsUrls.push(entry.name);
              }
            }
          }
        });
    ...
    window.postMessage({ type: "FPJS_TO_CONTENT_SCRIPT_REQUEST", result: fpjsDetected, urls: fpjsUrls});
  } catch (e) { 
    window.postMessage({ type: "FPJS_TO_CONTENT_SCRIPT_REQUEST", result: "FPJS_FAILED" }); 
  }
`;
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  if (message.msg === "FPJS_FETCH_REQUEST") {
    injectScript(fpjsDetect);
}
});
...
window.addEventListener(
  ...
  function (event) {
    ...
    if (event.data.type == "FPJS_TO_CONTENT_SCRIPT_REQUEST") {
    chrome.runtime.sendMessage({
      msg: "FPJS_TO_BACKGROUND_FROM_FETCH_REQUEST",
      data: event.data.result,
