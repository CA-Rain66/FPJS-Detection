...
/**
 * Fetches FPJS data, implemented following the same pattern above
 * 
 */

function fetchFPJSData() {
  return new Promise((resolve, reject) => {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      chrome.tabs.sendMessage(
        tabs[0].id,
        { msg: "FPJS_FETCH_REQUEST" },
        function (response) {
          function onResponseHandler(message, sender, sendResponse) {
            chrome.runtime.onMessage.removeListener(onResponseHandler);
            if (message.msg == "FPJS_TO_BACKGROUND_FROM_FETCH_REQUEST") {
              resolve(message);
            }
          }
          chrome.runtime.onMessage.addListener(onResponseHandler);
...
async function runAnalysis() {
  ...
  const fpjsData = await fetchFPJSData();
  ...
  if (fpjsData && fpjsData.detected !== undefined) {
    url = new URL(fpjsData.location);
    domain = parseURL(url);
    logData(domain, "FPJS", fpjsData);
  }
...
var analysisUserendSkeleton = () => {
  return {
    ...
    fpjs_before_gpc: null,
    fpjs_after_gpc: null,
    fpjs_urls_before_gpc: null,
    fpjs_urls_after_gpc: null, //might delete these
  };
};
...
function logData(domain, command, data) {
  ...
  if (command === "FPJS") {
    analysis[domain][callIndex][gpcStatusKey]["FPJS"] = [];
    analysis[domain][callIndex][gpcStatusKey]["FPJS"].push(data);

    // Detailed case for summary object
    if (gpcStatusKey == "BEFORE_GPC") {
      analysis_userend[domain]["fpjs_before_gpc"] = data["fpjsString"];
    }
    if (gpcStatusKey == "AFTER_GPC") {
      analysis_userend[domain]["fpjs_after_gpc"] = data["fpjsString"];
      //additionally I can include data.urls for audit proof of change
    }
  }
